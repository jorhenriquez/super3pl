<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-semibold text-gray-800">Validación de Pedido #{{ $pedido->id }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('validacion.finalizar', $pedido->id) }}" method="POST" onsubmit="return confirm('¿Finalizar validación de este pedido?')">
                    @csrf
                    @method('PATCH')
                ...
                <div class="p-6">
                    ...
                    <form action="{{ route('validacion.procesar', $pedido->id) }}" method="POST" autocomplete="off">
                        @csrf
                        <input 
                            type="text" 
                            name="codigo" 
                            autofocus 
                            placeholder="Escanea o escribe el código o EAN" 
                            class="w-full border px-4 py-2 rounded shadow"
                            onfocus="this.select()"
                        />
                    </form>

                    @if(session('status'))
                        <div class="mt-2 mb-4 py-4 text-green-800 bg-green-100 font-semibold border border-green-400 rounded text-center text-lg font-semibold">{{ session('status') }}</div>
                    @endif

                    @if(session('error'))
                        <div class="mt-2 mb-4 py-4 text-red-800 bg-red-100 font-semibold border border-red-400 rounded text-center text-lg font-semibold">{{ session('error') }}</div>
                    @endif

                    <div class="mt-8">
                        <h3 class="font-semibold text-lg mb-2">Líneas del Pedido</h3>
                        <table class="w-full table-auto border text-sm">
                            ...
                            <tbody>
                                @foreach($pedido->lineas as $linea)
                                    <tr class="border-t">
                                        <td class="px-4 py-2">{{ $linea->product->descripcion }}</td>
                                        <td class="px-4 py-2">{{ $linea->product->codigo }}</td>
                                        <td class="px-4 py-2">{{ $linea->product->ean }}</td>
                                        <td class="px-4 py-2">{{ $linea->cantidad_total }}</td>
                                        <td class="px-4 py-2">{{ $linea->cantidad_revisada }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
